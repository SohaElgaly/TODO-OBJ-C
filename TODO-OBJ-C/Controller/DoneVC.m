...
@property (strong, nonatomic) NSMutableArray<Task*> *doneDisplayArray;
@property (strong, nonatomic) NSUserDefaults *defaults;
@property (strong, nonatomic) NSMutableArray<Task*> *filteredTodoArray;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    Task *task = self.isSearching ? self.filteredTodoArray[indexPath.row] : [self tasksForSection:indexPath.section][indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    Task *selectedTask = self.isSearching ? self.filteredTodoArray[indexPath.row] : [self tasksForSection:indexPath.section][indexPath.row];
    
    EditVC *editVC = [self.storyboard instantiateViewControllerWithIdentifier:@"EdiVC"];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        Task *taskToDelete = self.isSearching ? self.filteredTodoArray[indexPath.row] : [self tasksForSection:indexPath.section][indexPath.row];
...
- (void)filterTasksForSearchText:(NSString *)searchText {
    ...
    NSArray<Task*> *filtered = [self.doneDisplayArray filteredArrayUsingPredicate:predicate];
